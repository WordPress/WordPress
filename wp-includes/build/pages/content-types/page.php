...
// Global storage for content-types routes and menu items
global $wp_content_types_routes, $wp_content_types_menu_items;
$wp_content_types_routes     = array();
$wp_content_types_menu_items = array();
...
/**
 * Register a menu item for the content-types page.
 *
 ...
 */
function wp_register_content_types_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
	global $wp_content_types_menu_items;
	...
	$wp_content_types_menu_items[] = $menu_item;
}
...
/**
 * Get all registered menu items for the content-types page.
 *
 * @return array Array of menu item objects.
 */
function wp_get_content_types_menu_items() {
	global $wp_content_types_menu_items;
	return $wp_content_types_menu_items ?? array();
}

/**
 * Preload REST API data for the content-types page.
 * Automatically called during page rendering.
 */
function wp_content_types_preload_data() {
...
/**
 * Render the content-types page.
 * Call this function from add_menu_page or add_submenu_page.
 */
function wp_content_types_render_page() {
	...
	wp_content_types_preload_data();

	// Get all registered routes and menu items
	$menu_items = wp_get_content_types_menu_items();
	$routes = wp_get_content_types_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'content-types-prerequisites', '', $asset['dependencies'], $asset['version'], true );

		// Add inline script to initialize the app
		$init_modules = [];
		wp_add_inline_script(
			'content-types-prerequisites',
			sprintf(
		...
		wp_register_style( 'content-types-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for content-types module
		...
		$boot_dependencies = apply_filters(
			'content-types_boot_dependencies',
			$boot_dependencies
		);
		...
		wp_register_script_module(
			'content-types',
			$build_constants['build_url'] . 'pages/content-types/loader.js',
			$boot_dependencies
		);

		// Enqueue the boot scripts and styles
		wp_enqueue_script( 'content-types-prerequisites' );
		wp_enqueue_script_module( 'content-types' );
		wp_enqueue_style( 'content-types-prerequisites' );
	}
	...
	<body class="content-types">
		<div id="content-types-app" style="height: 100vh; box-sizing: border-box;"></div>
	<?php
 ...
 */
function wp_content_types_intercept_render() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( isset( $_GET['page'] ) && 'content-types' === $_GET['page'] ) {
		wp_content_types_render_page();
		exit;
	}
}

// Hook the interceptor to admin_init
add_action( 'admin_init', 'wp_content_types_intercept_render' );
