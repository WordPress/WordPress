...
// Global storage for taxonomies routes and menu items
global $wp_taxonomies_routes, $wp_taxonomies_menu_items;
$wp_taxonomies_routes     = array();
$wp_taxonomies_menu_items = array();
...
/**
 * Register a menu item for the taxonomies page.
 *
 ...
 */
function wp_register_taxonomies_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
	global $wp_taxonomies_menu_items;
	...
	$wp_taxonomies_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the taxonomies page.
 *
 * @return array Array of route objects.
 */
function wp_get_taxonomies_routes() {
	global $wp_taxonomies_routes;
	return $wp_taxonomies_routes ?? array();
}

/**
 * Get all registered menu items for the taxonomies page.
 *
 * @return array Array of menu item objects.
 */
function wp_get_taxonomies_menu_items() {
	global $wp_taxonomies_menu_items;
	return $wp_taxonomies_menu_items ?? array();
}

/**
 * Preload REST API data for the taxonomies page.
 * Automatically called during page rendering.
 */
function wp_taxonomies_preload_data() {
...
/**
 * Render the taxonomies page.
 * Call this function from add_menu_page or add_submenu_page.
 */
function wp_taxonomies_render_page() {
	...
	wp_taxonomies_preload_data();

	// Get all registered routes and menu items
	$menu_items = wp_get_taxonomies_menu_items();
	$routes = wp_get_taxonomies_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'taxonomies-prerequisites', '', $asset['dependencies'], $asset['version'], true );

		// Add inline script to initialize the app
		$init_modules = [];
		wp_add_inline_script(
			'taxonomies-prerequisites',
			sprintf(
		...
		wp_register_style( 'taxonomies-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for taxonomies module
		...
		wp_enqueue_script( 'taxonomies-prerequisites' );
		wp_enqueue_script_module( 'taxonomies' );
		wp_enqueue_style( 'taxonomies-prerequisites' );
	}
	...
	<body class="taxonomies">
		<div id="taxonomies-app" style="height: 100vh; box-sizing: border-box;"></div>
	<?php
 ...
 */
function wp_taxonomies_intercept_render() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( isset( $_GET['page'] ) && 'taxonomies' === $_GET['page'] ) {
		wp_taxonomies_render_page();
		exit;
	}
}

// Hook the interceptor to admin_init
add_action( 'admin_init', 'wp_taxonomies_intercept_render' );
