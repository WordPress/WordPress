...
// Global storage for media-editor routes and menu items
global $wp_media_editor_routes, $wp_media_editor_menu_items;
$wp_media_editor_routes     = array();
$wp_media_editor_menu_items = array();

/**
 * Register a route for the media-editor page.
 *
 ...
 */
function wp_register_media_editor_route( $path, $content_module = null, $route_module = null ) {
	...
	$wp_media_editor_routes[] = $route;
}

/**
 * Register a menu item for the media-editor page.
 *
 ...
 */
function wp_register_media_editor_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
	global $wp_media_editor_menu_items;
	...
	$wp_media_editor_menu_items[] = $menu_item;
}
...
/**
 * Get all registered menu items for the media-editor page.
 *
 * @return array Array of menu item objects.
 */
function wp_get_media_editor_menu_items() {
	global $wp_media_editor_menu_items;
	return $wp_media_editor_menu_items ?? array();
}

/**
 * Preload REST API data for the media-editor page.
 * Automatically called during page rendering.
 */
function wp_media_editor_preload_data() {
 ...
 */
function wp_media_editor_render_page() {
	...
	wp_media_editor_preload_data();

	// Get all registered routes and menu items
	$menu_items = wp_get_media_editor_menu_items();
	$routes = wp_get_media_editor_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_style( 'media-editor-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for media-editor module
		...
		wp_register_script_module(
			'media-editor',
			$build_constants['build_url'] . 'pages/media-editor/loader.js',
			$boot_dependencies
		);

		// Enqueue the boot scripts and styles
		wp_enqueue_script( 'media-editor-prerequisites' );
		wp_enqueue_script_module( 'media-editor' );
		wp_enqueue_style( 'media-editor-prerequisites' );
	}
	...
	<body class="media-editor">
		<div id="media-editor-app" style="height: 100vh; box-sizing: border-box;"></div>
	<?php
 ...
 */
function wp_media_editor_intercept_render() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( isset( $_GET['page'] ) && 'media-editor' === $_GET['page'] ) {
		wp_media_editor_render_page();
		exit;
	}
}

// Hook the interceptor to admin_init
add_action( 'admin_init', 'wp_media_editor_intercept_render' );
