<?php
/**
 * Page: media-editor (wp-admin integrated)
...
global $wp_media_editor_wp_admin_routes, $wp_media_editor_wp_admin_menu_items;
$wp_media_editor_wp_admin_routes     = array();
$wp_media_editor_wp_admin_menu_items = array();

/**
 * Register a route for the media-editor-wp-admin page.
 *
 ...
 */
function wp_register_media_editor_wp_admin_route( $path, $content_module = null, $route_module = null ) {
	...
	$wp_media_editor_wp_admin_routes[] = $route;
}

/**
 * Register a menu item for the media-editor-wp-admin page.
 ...
 */
function wp_register_media_editor_wp_admin_menu_item( $id, $label, $to, $parent_id = '' ) {
	global $wp_media_editor_wp_admin_menu_items;
	...
	$wp_media_editor_wp_admin_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the media-editor-wp-admin page.
 *
 * @return array Array of route objects.
 */
function wp_get_media_editor_wp_admin_routes() {
	global $wp_media_editor_wp_admin_routes;
	return $wp_media_editor_wp_admin_routes ?? array();
}

/**
 * Get all registered menu items for the media-editor-wp-admin page.
 *
 * @return array Array of menu item objects.
 */
function wp_get_media_editor_wp_admin_menu_items() {
	global $wp_media_editor_wp_admin_menu_items;
	return $wp_media_editor_wp_admin_menu_items ?? array();
}

/**
 * Preload REST API data for the media-editor-wp-admin page.
 * Automatically called during page rendering.
 */
function wp_media_editor_wp_admin_preload_data() {
...
/**
 * Enqueue scripts and styles for the media-editor-wp-admin page.
 * Hooked to admin_enqueue_scripts.
 *
 * @param string $hook_suffix The current admin page.
 */
function wp_media_editor_wp_admin_enqueue_scripts( $hook_suffix ) {
	// Check all possible ways this page can be accessed:
	// 1. Menu page via admin.php?page=media-editor-wp-admin (plugin)
	// 2. Direct file via media-editor.php (Core) - screen ID will be 'media-editor'
	$current_screen = get_current_screen();
	$is_our_page = (
		( isset( $_GET['page'] ) && 'media-editor-wp-admin' === $_GET['page'] ) || // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		( $current_screen && 'media-editor' === $current_screen->id )
	);
	...
	wp_media_editor_wp_admin_preload_data();

	// Get all registered routes
	$routes = wp_get_media_editor_wp_admin_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'media-editor-wp-admin-prerequisites', '', $asset['dependencies'], $asset['version'], true );
		...
		JS;
		wp_add_inline_script(
			'media-editor-wp-admin-prerequisites',
			sprintf(
				'( %s )( %s, %s );',
				$init_js_function,
				wp_json_encode( 'media-editor-wp-admin-app', JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ),
		...
		wp_register_style( 'media-editor-wp-admin-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for media-editor-wp-admin module
		...
		$boot_dependencies = apply_filters(
			'media-editor-wp-admin_boot_dependencies',
			$boot_dependencies
		);
		...
		wp_enqueue_script( 'media-editor-wp-admin-prerequisites' );
		wp_enqueue_script_module( 'media-editor-wp-admin' );
		wp_enqueue_style( 'media-editor-wp-admin-prerequisites' );
	}
}

/**
 * Render the media-editor-wp-admin page.
 * Call this function from add_menu_page or add_submenu_page.
 * This renders within the normal WordPress admin interface.
 */
function wp_media_editor_wp_admin_render_page() {
	?>
	<style>
...
add_action( 'admin_enqueue_scripts', 'wp_media_editor_wp_admin_enqueue_scripts' );
