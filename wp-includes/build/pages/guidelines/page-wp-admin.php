<?php
/**
 * Page: guidelines (wp-admin integrated)
...
// Global storage for guidelines routes and menu items
global $wp_guidelines_wp_admin_routes, $wp_guidelines_wp_admin_menu_items;
$wp_guidelines_wp_admin_routes     = array();
$wp_guidelines_wp_admin_menu_items = array();

/**
 * Register a route for the guidelines-wp-admin page.
 *
 ...
 */
function wp_register_guidelines_wp_admin_route( $path, $content_module = null, $route_module = null ) {
	global $wp_guidelines_wp_admin_routes;
	...
	$wp_guidelines_wp_admin_routes[] = $route;
}

/**
 * Register a menu item for the guidelines-wp-admin page.
 ...
 */
function wp_register_guidelines_wp_admin_menu_item( $id, $label, $to, $parent_id = '' ) {
	global $wp_guidelines_wp_admin_menu_items;
	...
	$wp_guidelines_wp_admin_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the guidelines-wp-admin page.
 *
 * @return array Array of route objects.
 */
function wp_get_guidelines_wp_admin_routes() {
	global $wp_guidelines_wp_admin_routes;
	return $wp_guidelines_wp_admin_routes ?? array();
}

/**
 * Get all registered menu items for the guidelines-wp-admin page.
 *
 * @return array Array of menu item objects.
 */
function wp_get_guidelines_wp_admin_menu_items() {
	global $wp_guidelines_wp_admin_menu_items;
	return $wp_guidelines_wp_admin_menu_items ?? array();
}

/**
 * Preload REST API data for the guidelines-wp-admin page.
 * Automatically called during page rendering.
 */
function wp_guidelines_wp_admin_preload_data() {
...
/**
 * Enqueue scripts and styles for the guidelines-wp-admin page.
 * Hooked to admin_enqueue_scripts.
 *
 * @param string $hook_suffix The current admin page.
 */
function wp_guidelines_wp_admin_enqueue_scripts( $hook_suffix ) {
	// Check all possible ways this page can be accessed:
	// 1. Menu page via admin.php?page=guidelines-wp-admin (plugin)
	// 2. Direct file via guidelines.php (Core) - screen ID will be 'guidelines'
	$current_screen = get_current_screen();
	$is_our_page = (
		( isset( $_GET['page'] ) && 'guidelines-wp-admin' === $_GET['page'] ) || // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		( $current_screen && 'guidelines' === $current_screen->id )
	);
	...
	wp_guidelines_wp_admin_preload_data();

	// Get all registered routes
	$routes = wp_get_guidelines_wp_admin_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'guidelines-wp-admin-prerequisites', '', $asset['dependencies'], $asset['version'], true );
		...
		JS;
		wp_add_inline_script(
			'guidelines-wp-admin-prerequisites',
			sprintf(
				'( %s )( %s, %s );',
				$init_js_function,
				wp_json_encode( 'guidelines-wp-admin-app', JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ),
		...
		wp_register_style( 'guidelines-wp-admin-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for guidelines-wp-admin module
		...
		$boot_dependencies = apply_filters(
			'guidelines-wp-admin_boot_dependencies',
			$boot_dependencies
		);
		...
		wp_register_script_module(
			'guidelines-wp-admin',
			$build_constants['build_url'] . 'pages/guidelines/loader.js',
			$boot_dependencies
		);

		// Enqueue the boot scripts and styles
		wp_enqueue_script( 'guidelines-wp-admin-prerequisites' );
		wp_enqueue_script_module( 'guidelines-wp-admin' );
		wp_enqueue_style( 'guidelines-wp-admin-prerequisites' );
	}
}

/**
 * Render the guidelines-wp-admin page.
 * Call this function from add_menu_page or add_submenu_page.
 * This renders within the normal WordPress admin interface.
 */
function wp_guidelines_wp_admin_render_page() {
	?>
	<style>
	...
	<div id="guidelines-wp-admin-app" class="boot-layout-container"></div>
	<?php
}

// Hook the enqueue function to admin_enqueue_scripts
add_action( 'admin_enqueue_scripts', 'wp_guidelines_wp_admin_enqueue_scripts' );
