...
global $wp_guidelines_routes, $wp_guidelines_menu_items;
$wp_guidelines_routes     = array();
$wp_guidelines_menu_items = array();

/**
 * Register a route for the guidelines page.
 *
 ...
 */
function wp_register_guidelines_route( $path, $content_module = null, $route_module = null ) {
	global $wp_guidelines_routes;
	...
	$wp_guidelines_routes[] = $route;
}

/**
 * Register a menu item for the guidelines page.
 *
 ...
 */
function wp_register_guidelines_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
	global $wp_guidelines_menu_items;
	...
	$wp_guidelines_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the guidelines page.
 *
 * @return array Array of route objects.
 */
function wp_get_guidelines_routes() {
	global $wp_guidelines_routes;
	return $wp_guidelines_routes ?? array();
}

/**
 * Get all registered menu items for the guidelines page.
 *
 * @return array Array of menu item objects.
 */
function wp_get_guidelines_menu_items() {
	global $wp_guidelines_menu_items;
	return $wp_guidelines_menu_items ?? array();
}

/**
 * Preload REST API data for the guidelines page.
 * Automatically called during page rendering.
 */
function wp_guidelines_preload_data() {
 ...
 */
function wp_guidelines_render_page() {
	...
	wp_guidelines_preload_data();

	// Get all registered routes and menu items
	$menu_items = wp_get_guidelines_menu_items();
	$routes = wp_get_guidelines_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'guidelines-prerequisites', '', $asset['dependencies'], $asset['version'], true );
		...
		wp_register_style( 'guidelines-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for guidelines module
		...
		wp_enqueue_script( 'guidelines-prerequisites' );
		wp_enqueue_script_module( 'guidelines' );
		wp_enqueue_style( 'guidelines-prerequisites' );
	}
	...
	<body class="guidelines">
		<div id="guidelines-app" style="height: 100vh; box-sizing: border-box;"></div>
	<?php
 ...
 */
function wp_guidelines_intercept_render() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( isset( $_GET['page'] ) && 'guidelines' === $_GET['page'] ) {
		wp_guidelines_render_page();
		exit;
	}
}

// Hook the interceptor to admin_init
add_action( 'admin_init', 'wp_guidelines_intercept_render' );
