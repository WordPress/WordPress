...
global $wp_experiments_routes, $wp_experiments_menu_items;
$wp_experiments_routes     = array();
$wp_experiments_menu_items = array();

/**
 * Register a route for the experiments page.
 *
 ...
 */
function wp_register_experiments_route( $path, $content_module = null, $route_module = null ) {
	...
	$wp_experiments_routes[] = $route;
}

/**
 * Register a menu item for the experiments page.
 *
 ...
 */
function wp_register_experiments_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
	global $wp_experiments_menu_items;
	...
	$wp_experiments_menu_items[] = $menu_item;
}
...
/**
 * Get all registered menu items for the experiments page.
 *
 * @return array Array of menu item objects.
 */
function wp_get_experiments_menu_items() {
	global $wp_experiments_menu_items;
	return $wp_experiments_menu_items ?? array();
}

/**
 * Preload REST API data for the experiments page.
 * Automatically called during page rendering.
 */
function wp_experiments_preload_data() {
 ...
 */
function wp_experiments_render_page() {
	...
	wp_experiments_preload_data();

	// Get all registered routes and menu items
	$menu_items = wp_get_experiments_menu_items();
	$routes = wp_get_experiments_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'experiments-prerequisites', '', $asset['dependencies'], $asset['version'], true );
		...
		wp_register_style( 'experiments-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for experiments module
		...
		wp_enqueue_script( 'experiments-prerequisites' );
		wp_enqueue_script_module( 'experiments' );
		wp_enqueue_style( 'experiments-prerequisites' );
	}
	...
	<body class="experiments">
		<div id="experiments-app" style="height: 100vh; box-sizing: border-box;"></div>
	<?php
 ...
 */
function wp_experiments_intercept_render() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( isset( $_GET['page'] ) && 'experiments' === $_GET['page'] ) {
		wp_experiments_render_page();
		exit;
	}
}

// Hook the interceptor to admin_init
add_action( 'admin_init', 'wp_experiments_intercept_render' );
