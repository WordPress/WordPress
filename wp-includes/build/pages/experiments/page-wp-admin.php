<?php
/**
 * Page: experiments (wp-admin integrated)
...
// Global storage for experiments routes and menu items
global $wp_experiments_wp_admin_routes, $wp_experiments_wp_admin_menu_items;
$wp_experiments_wp_admin_routes     = array();
$wp_experiments_wp_admin_menu_items = array();

/**
 * Register a route for the experiments-wp-admin page.
 *
 ...
 */
function wp_register_experiments_wp_admin_route( $path, $content_module = null, $route_module = null ) {
	global $wp_experiments_wp_admin_routes;
	...
	$wp_experiments_wp_admin_routes[] = $route;
}

/**
 * Register a menu item for the experiments-wp-admin page.
 ...
 */
function wp_register_experiments_wp_admin_menu_item( $id, $label, $to, $parent_id = '' ) {
	global $wp_experiments_wp_admin_menu_items;
	...
	$wp_experiments_wp_admin_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the experiments-wp-admin page.
 *
 * @return array Array of route objects.
 */
function wp_get_experiments_wp_admin_routes() {
	global $wp_experiments_wp_admin_routes;
	return $wp_experiments_wp_admin_routes ?? array();
}

/**
 * Get all registered menu items for the experiments-wp-admin page.
 *
 * @return array Array of menu item objects.
 */
function wp_get_experiments_wp_admin_menu_items() {
	global $wp_experiments_wp_admin_menu_items;
	return $wp_experiments_wp_admin_menu_items ?? array();
}

/**
 * Preload REST API data for the experiments-wp-admin page.
 * Automatically called during page rendering.
 */
function wp_experiments_wp_admin_preload_data() {
...
/**
 * Enqueue scripts and styles for the experiments-wp-admin page.
 * Hooked to admin_enqueue_scripts.
 *
 * @param string $hook_suffix The current admin page.
 */
function wp_experiments_wp_admin_enqueue_scripts( $hook_suffix ) {
	// Check all possible ways this page can be accessed:
	// 1. Menu page via admin.php?page=experiments-wp-admin (plugin)
	// 2. Direct file via experiments.php (Core) - screen ID will be 'experiments'
	$current_screen = get_current_screen();
	$is_our_page = (
		( isset( $_GET['page'] ) && 'experiments-wp-admin' === $_GET['page'] ) || // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		( $current_screen && 'experiments' === $current_screen->id )
	);
	...
	do_action( 'experiments-wp-admin_init' );

	// Preload REST API data
	wp_experiments_wp_admin_preload_data();

	// Get all registered routes
	$routes = wp_get_experiments_wp_admin_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'experiments-wp-admin-prerequisites', '', $asset['dependencies'], $asset['version'], true );
		...
		wp_add_inline_script(
			...
			sprintf(
				'( %s )( %s, %s );',
				$init_js_function,
				wp_json_encode( 'experiments-wp-admin-app', JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ),
		...
		wp_register_style( 'experiments-wp-admin-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for experiments-wp-admin module
		...
		$boot_dependencies = apply_filters(
			'experiments-wp-admin_boot_dependencies',
			$boot_dependencies
		);
		...
		wp_register_script_module(
			...
			$build_constants['build_url'] . 'pages/experiments/loader.js',
			$boot_dependencies
		);

		// Enqueue the boot scripts and styles
		wp_enqueue_script( 'experiments-wp-admin-prerequisites' );
		wp_enqueue_script_module( 'experiments-wp-admin' );
		wp_enqueue_style( 'experiments-wp-admin-prerequisites' );
	}
}

/**
 * Render the experiments-wp-admin page.
 * Call this function from add_menu_page or add_submenu_page.
 * This renders within the normal WordPress admin interface.
 */
function wp_experiments_wp_admin_render_page() {
	?>
	<style>
...
add_action( 'admin_enqueue_scripts', 'wp_experiments_wp_admin_enqueue_scripts' );
