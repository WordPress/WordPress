...
__webpack_require__.d(__webpack_exports__, {
  ...
  Kr: () => (/* reexport */ useMemo),
  li: () => (/* reexport */ hooks_module_A),
  J0: () => (/* reexport */ hooks_module_h),
...
function useInit(callback) {
  hooks_module_y(withScope(callback), []);
}
...
function useCallback(callback, inputs) {
  return hooks_module_q(withScope(callback), inputs);
}
...
function useMemo(factory, inputs) {
  return hooks_module_T(withScope(factory), inputs);
}
...
const Directives = ({
  ...
  const scope = hooks_module_A({}).current;
  scope.evaluate = hooks_module_q(getEvaluate({
    scope
  }), []);
  const {
    client,
    server
  } = hooks_module_x(context);
  scope.context = client;
  scope.serverContext = server;
  /* eslint-disable react-hooks/rules-of-hooks */
  scope.ref = previousScope?.ref || hooks_module_A(null);
...
/* harmony default export */ const directives = (() => {
  ...
  directive('context', ({
    ...
    } = hooks_module_x(inheritedContext);
    const ns = defaultEntry.namespace;
    const client = hooks_module_A(proxifyState(ns, {}));
    const server = hooks_module_A(proxifyState(ns, {}, {
      readOnly: true
    }));

    // No change should be made if `defaultEntry` does not exist.
    const contextStack = hooks_module_T(() => {
      const result = {
        client: {
          ...inheritedClient
        },
  ...
  directive('ignore', ({
    ...
    const cached = hooks_module_T(() => innerHTML, []);
    return _(Type, {
  ...
  directive('each', ({
    ...
    const inheritedValue = hooks_module_x(inheritedContext);
    const [entry] = each;
    const {
      namespace,
      suffix
