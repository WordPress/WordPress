 ...
 * @internal
 */
class HttpClient implements \Sentry\HttpClient\HttpClientInterface
{
    /**
    ...
    public function sendRequest(\Sentry\HttpClient\Request $request, \Sentry\Options $options) : \Sentry\HttpClient\Response
    {
        ...
        $requestHeaders = \Sentry\Util\Http::getRequestHeaders($dsn, $this->sdkIdentifier, $this->sdkVersion);
        ...
        $responseHeaderCallback = function ($curlHandle, $headerLine) use(&$responseHeaders) : int {
            return \Sentry\Util\Http::parseResponseHeaders($headerLine, $responseHeaders);
        };
        ...
        if ($httpSslNativeCa) {
            if (\defined('CURLSSLOPT_NATIVE_CA') && isset(\curl_version()['version']) && \version_compare(\curl_version()['version'], '7.71', '>=')) {
        ...
        if ($body === \false) {
            ...
            return new \Sentry\HttpClient\Response(0, [], $message);
        }
        ...
        return new \Sentry\HttpClient\Response($statusCode, $responseHeaders, $error);
    }
}
