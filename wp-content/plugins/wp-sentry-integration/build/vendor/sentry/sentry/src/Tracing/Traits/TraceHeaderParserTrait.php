...
trait TraceHeaderParserTrait
{
    ...
    {
        $result = ['traceId' => null, 'parentSpanId' => null, 'parentSampled' => null, 'dynamicSamplingContext' => null, 'sampleRand' => null, 'parentSamplingRate' => null];
        $hasSentryTrace = \false;
        if (\preg_match(self::$sentryTraceparentHeaderRegex, $sentryTrace, $matches)) {
            if (!empty($matches['trace_id'])) {
                $result['traceId'] = new \Sentry\Tracing\TraceId($matches['trace_id']);
                $hasSentryTrace = \true;
            }
            if (!empty($matches['span_id'])) {
                $result['parentSpanId'] = new \Sentry\Tracing\SpanId($matches['span_id']);
                $hasSentryTrace = \true;
            }
