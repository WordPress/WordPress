...
final class StacktraceBuilder
{
    ...
    {
        $this->frameBuilder = new \Sentry\FrameBuilder($options, $representationSerializer);
        if (\Sentry\Util\PHPConfiguration::isBooleanIniOptionEnabled('zend.exception_ignore_args')) {
     ...
     */
    public function buildFromException(\Throwable $exception) : \Sentry\Stacktrace
    {
     ...
     */
    public function buildFromBacktrace(array $backtrace, string $file, int $line) : \Sentry\Stacktrace
    {
        $frames = [];
        foreach ($backtrace as $backtraceFrame) {
            ...
            $file = $backtraceFrame['file'] ?? \Sentry\Frame::INTERNAL_FRAME_FILENAME;
        ...
        return new \Sentry\Stacktrace($frames);
    }
}
