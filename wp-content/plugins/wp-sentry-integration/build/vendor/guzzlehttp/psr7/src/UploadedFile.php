...
class UploadedFile implements \WPSentry\ScopedVendor\Psr\Http\Message\UploadedFileInterface
{
    ...
    private function setStreamOrFile($streamOrFile) : void
    {
        if (\is_string($streamOrFile)) {
            ...
            $this->stream = new \WPSentry\ScopedVendor\GuzzleHttp\Psr7\Stream($streamOrFile);
        } elseif ($streamOrFile instanceof \WPSentry\ScopedVendor\Psr\Http\Message\StreamInterface) {
    ...
    {
        if (!isset(\WPSentry\ScopedVendor\GuzzleHttp\Psr7\UploadedFile::ERROR_MAP[$error])) {
    ...
    {
        $this->validateActive();
        if ($this->stream instanceof \WPSentry\ScopedVendor\Psr\Http\Message\StreamInterface) {
            return $this->stream;
        }
        /** @var string $file */
        $file = $this->file;
        return new \WPSentry\ScopedVendor\GuzzleHttp\Psr7\LazyOpenStream($file, 'r+');
    }
    public function moveTo($targetPath) : void
    {
        ...
        if ($this->file) {
            ...
            \WPSentry\ScopedVendor\GuzzleHttp\Psr7\Utils::copyToStream($this->getStream(), new \WPSentry\ScopedVendor\GuzzleHttp\Psr7\LazyOpenStream($targetPath, 'w'));
            $this->moved = \true;
        }
        if (\false === $this->moved) {
