...
class ScheduledReports extends \Piwik\Plugin
{
    ...
    {
        $idReports = \Piwik\Plugins\ScheduledReports\API::getInstance()->getReports($idSite);
        foreach ($idReports as $report) {
            $idReport = $report['idreport'];
            \Piwik\Plugins\ScheduledReports\API::getInstance()->deleteReport($idReport);
        }
    }
    ...
    public function getRendererInstance(&$reportRenderer, $reportType, $outputType, $report)
    {
        ...
        if ($reportFormat == ReportRenderer::HTML_FORMAT) {
            $reportRenderer->setRenderImageInline($outputType != \Piwik\Plugins\ScheduledReports\API::OUTPUT_SAVE_ON_DISK);
        }
    }
     ...
     */
    public function sendReport($reportType, $report, $contents, $filename, $prettyDate, $reportSubject, $reportTitle, $additionalFiles, $period, $force)
    {
        if (!self::manageEvent($reportType)) {
            return;
        }
        ...
        $reportGenerator = StaticContainer::get(\Piwik\Plugins\ScheduledReports\ReportEmailGenerator::class . '.' . $reportFormat);
        ...
        foreach ($reports as $report) {
            if ($report['type'] == MobileMessaging::MOBILE_TYPE) {
                ...
                if (count($updatedPhoneNumbers) != count($reportPhoneNumbers)) {
                    ...
                    $api->updateReport($report['idreport'], $report['idsite'], $report['description'], $report['period'], $report['hour'], $report['type'], $report['format'], $report['reports'], $reportParameters);
                }
            }
        }
    }
    ...
    {
        \Piwik\Plugins\ScheduledReports\Model::install();
        \Piwik\Plugins\ScheduledReports\SubscriptionModel::install();
    }
    ...
    {
        return array(Schedule::PERIOD_NEVER => Piwik::translate('General_Never'), Schedule::PERIOD_DAY => Piwik::translate('General_Daily'), Schedule::PERIOD_WEEK => Piwik::translate('General_Weekly'), Schedule::PERIOD_MONTH => Piwik::translate('General_Monthly'));
    }
    public static function getPeriodFrequencyTranslations()
    {
        return [Schedule::PERIOD_DAY => ['single' => Piwik::translate('Intl_PeriodDay'), 'plural' => Piwik::translate('Intl_PeriodDays')], Schedule::PERIOD_WEEK => ['single' => Piwik::translate('Intl_PeriodWeek'), 'plural' => Piwik::translate('Intl_PeriodWeeks')], Schedule::PERIOD_MONTH => ['single' => Piwik::translate('Intl_PeriodMonth'), 'plural' => Piwik::translate('Intl_PeriodMonths')]];
    }
