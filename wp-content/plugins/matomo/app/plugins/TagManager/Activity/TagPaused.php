...
class TagPaused extends \Piwik\Plugins\TagManager\Activity\TagBaseActivity
{
    protected $eventName = 'TagManager.pauseContainerTag.end';
    public function extractParams($eventData)
    {
        if (!$this->hasRequestedApiMethod('pauseContainerTag')) {
            return \false;
        }
    ...
    public function getTranslatedDescription($activityData, $performingUser)
    {
        ...
        $desc = sprintf('paused the tag "%1$s" from container "%2$s" for site "%3$s"', $entityName, $containerName, $siteName);
        return $desc;
    }
}
