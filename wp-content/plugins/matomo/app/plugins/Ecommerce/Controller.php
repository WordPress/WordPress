...
class Controller extends \Piwik\Plugins\Goals\Controller
{
    ...
    private function addSparklineEvolutionValues(array $return, $idGoal, string $date, string $lastPeriodDate, DataTable\Row $currentDataRow, DataTable\Row $previousDataRow) : array
    {
        ...
        if ($idGoal == Piwik::LABEL_ID_GOAL_IS_ECOMMERCE_ORDER) {
            $metrics = array_merge($metrics, ['items' => Piwik::translate('General_PurchasedProducts'), 'avg_order_revenue' => Piwik::translate('General_AverageOrderValue')]);
        }
        ...
        foreach ($return as $columnName => $value) {
            if (array_key_exists($columnName, $metrics) && array_key_exists($columnName, $return)) {
                ...
                $return[$columnName . '_tooltip'] = Piwik::translate('General_EvolutionSummaryGeneric', array($currentValueFormatted . ' ' . Piwik::translate($metricTranslationKey), $currentPrettyDate, $pastValueFormatted . ' ' . Piwik::translate($metricTranslationKey), $lastPrettyDate, $trend));
            }
        }
        return $return;
    }
    public function getConversionsOverview()
    {
        ...
        $goalMetrics = Request::processRequest('Goals.get', ['idGoal' => $idGoal, 'idSite' => $this->idSite, 'date' => $date, 'period' => $period, 'segment' => Common::unsanitizeInputValue($segment), 'filter_limit' => '-1'], $default = []);
