...
class FixDuplicateLogActions extends ConsoleCommand
{
    ...
    protected function configure()
    {
        ...
        $this->addNoValueOption('invalidate-archives', null, "If supplied, archives for logs that use duplicate actions will be invalidated." . " On the next cron archive run, the reports for those dates will be re-processed.");
        $this->setDescription('Removes duplicates in the log action table and fixes references to the duplicates in ' . 'related tables. NOTE: This action can take a long time to run!');
    }
    protected function doExecute() : int
    {
        ...
        $this->writeSuccessMessage(array("Found and deleted {$numberRemoved} duplicate action entries in the {$logActionTable} table.", "References in log_link_visit_action, log_conversion and log_conversion_item were corrected.", $timer->__toString()));
        return self::SUCCESS;
    }
    ...
    private function fixDuplicateActionReferences($duplicateActions)
    {
        ...
        foreach ($duplicateActions as $index => $dupeInfo) {
            ...
            $this->logger->debug("  idaction = {idaction}, duplicate idactions = {duplicateIdActions}", array('idaction' => $toIdAction, 'duplicateIdActions' => $fromIdActions));
