...
class WidgetMetadata
{
    ...
    public function getWidgetMetadata(CategoryList $categoryList, WidgetsList $widgetsList)
    {
        ...
        foreach ($widgetsList->getWidgetConfigs() as $widgetConfig) {
            ...
            foreach ($widgets as $widget) {
                ...
                if ($widgetConfig instanceof WidgetContainerConfig && $widgetConfig->getLayout() == CoreHome::WIDGET_CONTAINER_LAYOUT_BY_DIMENSION) {
                    $metadataOverrides = ['category' => $widgetConfig->getCategoryId(), 'subcategory' => $widgetConfig->getSubcategoryId(), 'name' => Piwik::translate($widget->getCategoryId()) . ': ' . Piwik::translate($widget->getName())];
                }
    ...
    private function buildSubcategoryMetadata($subcategory)
    {
        ...
        return array('id' => (string) $subcategory->getId(), 'name' => Piwik::translate($subcategory->getName()), 'order' => $subcategory->getOrder(), 'help' => Piwik::translate($subcategory->getHelp()));
    }
    /**
