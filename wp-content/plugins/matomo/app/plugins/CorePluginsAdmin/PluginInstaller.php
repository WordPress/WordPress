...
class PluginInstaller
{
    ...
    {
        $dirs = array(StaticContainer::get('path.tmp') . self::PATH_TO_DOWNLOAD, Manager::getPluginsDirectory());
    ...
    private function downloadPluginFromMarketplace()
    {
        try {
            ...
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException($errorMessage);
        }
    }
    /**
     * @param $pluginZipFile
    ...
    private function extractPluginFiles($pluginZipFile, $pathExtracted)
    {
        ...
        if (0 == ($pluginFiles = $archive->extract($pathExtracted))) {
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException(Piwik::translate('CoreUpdater_ExceptionArchiveIncompatible', $archive->errorInfo()));
        }
        if (0 == count($pluginFiles)) {
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException(Piwik::translate('Plugin Zip File Is Empty'));
        }
    }
    private function makeSurePluginJsonExists($tmpPluginFolder)
    {
        ...
        if (!file_exists($pluginJsonPath)) {
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException('Plugin is not valid, it is missing the plugin.json file.');
        }
    }
    private function makeSureThereAreNoMissingRequirements($metadata)
    {
        ...
        if (!empty($missingDependencies)) {
            ...
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException($message);
        }
    }
    private function getPluginMetadataIfValid($tmpPluginFolder)
    {
        ...
        if (empty($metadata)) {
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException('Plugin is not valid, plugin.json is empty or does not contain valid JSON.');
        }
        if (empty($metadata->name)) {
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException('Plugin is not valid, the plugin.json file does not specify the plugin name.');
        }
        if (!preg_match('/^[a-zA-Z0-9_-]+$/', $metadata->name)) {
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException('The plugin name specified in plugin.json contains illegal characters. ' . 'Plugin name can only contain following characters: [a-zA-Z0-9-_].');
        }
        if (empty($metadata->version)) {
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException('Plugin is not valid, the plugin.json file does not specify the plugin version.');
        }
        if (empty($metadata->description)) {
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException('Plugin is not valid, the plugin.json file does not specify a description.');
        }
        return $metadata;
    }
    ...
    private function getNameOfFirstSubfolder($pluginDir)
    {
        ...
        if (empty($firstSubFolder)) {
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException('The plugin ZIP file does not contain a subfolder, but Piwik expects plugin files to be within a subfolder in the Zip archive.');
        }
    ...
    private function makeSurePluginNameIsValid()
    {
        try {
            ...
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException($e->getMessage());
        }
        if (empty($pluginDetails)) {
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException('This plugin was not found in the Marketplace.');
        }
    }
    private function checkMarketplaceIsEnabled()
    {
        if (!isset($this->marketplaceClient)) {
            throw new \Piwik\Plugins\CorePluginsAdmin\PluginInstallerException('Marketplace plugin needs to be enabled to perform this action.');
        }
    }
