...
namespace {
    ...
    return array('ini.log.log_writers' => \Piwik\DI::decorate(function ($previous, Container $c) {
        if (\isTrackerDebugEnabled($c) && \Piwik\Common::isPhpCliMode()) {
            $previous[] = 'screen';
            $previous = \array_unique($previous);
        }
        return $previous;
    }), 'log.handler.classes' => \Piwik\DI::decorate(function ($previous, Container $c) {
        if (isset($previous['screen']) && \isTrackerDebugEnabled($c)) {
