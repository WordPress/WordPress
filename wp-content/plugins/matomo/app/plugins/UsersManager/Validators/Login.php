...
class Login extends BaseValidator
{
    ...
    {
        if (!SettingsPiwik::isUserCredentialsSanityCheckEnabled() && !empty($value)) {
            return;
        }
        $l = strlen($value);
        if (!($l >= self::LOGIN_MIN_LENGTH && $l <= self::LOGIN_MAX_LENGTH && preg_match('/^[A-Za-zÄäÖöÜüß0-9_.@+-]*$/D', $value) > 0)) {
            throw new Exception(Piwik::translate('UsersManager_ExceptionInvalidLoginFormat', [self::LOGIN_MIN_LENGTH, self::LOGIN_MAX_LENGTH]));
        }
        if ($this->checkUnique) {
            $this->isUnique($value);
        }
