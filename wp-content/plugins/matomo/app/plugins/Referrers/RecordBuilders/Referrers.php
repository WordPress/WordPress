...
class Referrers extends RecordBuilder
{
    ...
    {
        $columns = [Metrics::INDEX_NB_UNIQ_VISITORS => (int) $row[Metrics::INDEX_NB_UNIQ_VISITORS], Metrics::INDEX_NB_VISITS => (int) $row[Metrics::INDEX_NB_VISITS], Metrics::INDEX_NB_ACTIONS => (int) $row[Metrics::INDEX_NB_ACTIONS], Metrics::INDEX_NB_USERS => (int) $row[Metrics::INDEX_NB_USERS], Metrics::INDEX_MAX_ACTIONS => (int) $row[Metrics::INDEX_MAX_ACTIONS], Metrics::INDEX_SUM_VISIT_LENGTH => (int) $row[Metrics::INDEX_SUM_VISIT_LENGTH], Metrics::INDEX_BOUNCE_COUNT => (int) $row[Metrics::INDEX_BOUNCE_COUNT], Metrics::INDEX_NB_VISITS_CONVERTED => (int) $row[Metrics::INDEX_NB_VISITS_CONVERTED]];
    ...
    protected function aggregateFromConversions(LogAggregator $logAggregator, array $reports, array $dimensions) : void
    {
        ...
        while ($row = $query->fetch()) {
            ...
            $columns = [Metrics::INDEX_GOALS => [$idGoal => Metrics::makeGoalColumnsRow($idGoal, $row)]];
