...
class HtmlTable extends Visualization
{
    ...
    {
        return new \Piwik\Plugins\CoreVisualizations\Visualizations\HtmlTable\Config();
    }
    public static function getDefaultRequestConfig()
    {
        return new \Piwik\Plugins\CoreVisualizations\Visualizations\HtmlTable\RequestConfig();
    }
    public function beforeLoadDataTable()
    {
        ...
        if ($this->isComparing()) {
            ...
            if (!empty($request['comparePeriods']) && count($request['comparePeriods']) == 1) {
    ...
    public function beforeRender()
    {
        ...
        if ($this->isComparing() && !empty($this->dataTable)) {
    ...
    public function afterGenericFiltersAreAppliedToLoadedDataTable()
    {
        ...
        // this must be done before metrics are formatted
    }
    ...
    private function getSiteSummary()
    {
        if (empty($this->siteSummary)) {
            ...
            $this->siteSummary = ApiRequest::processRequest('API.get', ['module' => 'API', 'action' => 'get', 'filter_limit' => '-1', 'disable_generic_filters' => 1, 'filter_offset' => 0, 'date' => Common::getRequestVar('date', null, 'string'), 'idSite' => Common::getRequestVar('idSite', null, 'int'), 'period' => $period, 'showColumns' => implode(',', $this->config->columns_to_display), 'columns' => implode(',', $this->config->columns_to_display)], $default = []);
        }
