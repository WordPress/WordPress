...
use Piwik\Updater;
use Piwik\Version;
class GenerateUpdate extends \Piwik\Plugins\CoreConsole\Commands\GeneratePluginBase
{
    protected function configure()
    {
        $this->setName('generate:update')->setDescription('Adds a new update to an existing plugin or "core"')->addRequiredValueOption('component', null, 'The name of an existing plugin or "core"');
    }
    protected function doExecute() : int
    {
        ...
        $replace = array('Piwik\\Plugins\\ExamplePlugin\\Updates' => $namespace, 'ExamplePlugin' => $component, 'Updates_0_0_2' => $className, '0.0.2' => $version);
        $whitelistFiles = array('/Updates', '/Updates/0.0.2.php');
        $this->copyTemplateToPlugin($exampleFolder, $component, $replace, $whitelistFiles);
        $this->writeSuccessMessage(array(sprintf('Updates/%s.php for %s generated.', $version, $component), 'You should have a look at the method update() or getSql() now.', 'Enjoy!'));
        return self::SUCCESS;
    }
    ...
    private function getComponent()
    {
        ...
        if (empty($component)) {
            $component = $this->askAndValidate('Enter the name of your plugin or "core": ', $validate, null, $components);
        } else {
            $validate($component);
        }
        return $component;
