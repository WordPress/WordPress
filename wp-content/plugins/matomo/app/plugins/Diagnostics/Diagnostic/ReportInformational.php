...
class ReportInformational implements \Piwik\Plugins\Diagnostics\Diagnostic\Diagnostic
{
    ...
    public function execute()
    {
        ...
        if (SettingsPiwik::isMatomoInstalled()) {
            $results[] = \Piwik\Plugins\Diagnostics\Diagnostic\DiagnosticResult::informationalResult('Had visits in last 1 day', $this->hadVisitsInLastDays(1));
            $results[] = \Piwik\Plugins\Diagnostics\Diagnostic\DiagnosticResult::informationalResult('Had visits in last 3 days', $this->hadVisitsInLastDays(3));
            $results[] = \Piwik\Plugins\Diagnostics\Diagnostic\DiagnosticResult::informationalResult('Had visits in last 5 days', $this->hadVisitsInLastDays(5));
            $archiveStart = Option::get(CronArchive::OPTION_ARCHIVING_STARTED_TS);
            $results[] = \Piwik\Plugins\Diagnostics\Diagnostic\DiagnosticResult::informationalResult('Archive Time Last Started', $archiveStart ? date("Y-m-d H:i:s", $archiveStart) : '-');
            $archiveEnd = Option::get(CronArchive::OPTION_ARCHIVING_FINISHED_TS);
            $results[] = \Piwik\Plugins\Diagnostics\Diagnostic\DiagnosticResult::informationalResult('Archive Time Last Finished', $archiveEnd ? date("Y-m-d H:i:s", $archiveEnd) : '-');
        }
        return $results;
    }
