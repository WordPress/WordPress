...
class Controller extends \Piwik\Plugin\Controller
{
    ...
    public function loginTwoFactorAuth()
    {
        ...
        $form = new \Piwik\Plugins\TwoFactorAuth\FormTwoFactorAuthCode();
        ...
        if ($form->validate()) {
            ...
            if ($nonce && $messageNoAccess === "" && $form->validate()) {
                ...
                if ($authCode && is_string($authCode)) {
                    ...
                    $authCode = strtoupper($authCode);
                    // recovery codes are stored upper case, app codes are only numbers
    ...
    {
        $this->validator->checkCanUseTwoFa();
        return $this->renderTemplate('userSettings', array('isEnabled' => \Piwik\Plugins\TwoFactorAuth\TwoFactorAuthentication::isUserUsingTwoFactorAuthentication(Piwik::getCurrentUserLogin()), 'isForced' => $this->twoFa->isUserRequiredToHaveTwoFactorEnabled(), 'disableNonce' => Nonce::getNonce(self::DISABLE_2FA_NONCE)));
    }
    ...
    public function setupTwoFactorAuth($standalone = \false)
    {
        ...
        if (!empty($secret) && !empty($authCode) && Nonce::verifyNonce(self::AUTH_CODE_NONCE, $authCodeNonce)) {
            if ($this->twoFa->validateAuthCodeDuringSetup(trim($authCode), $secret)) {
                ...
                $container = StaticContainer::getContainer();
                $email = $container->make(TwoFactorAuthEnabledEmail::class, array('login' => Piwik::getCurrentUserLogin(), 'emailAddress' => Piwik::getCurrentUserEmail()));
                $email->safeSend();
                if ($standalone) {
        ...
        if (!$this->recoveryCodeDao->getAllRecoveryCodesForLogin($login) || !$hasSubmittedForm && !\Piwik\Plugins\TwoFactorAuth\TwoFactorAuthentication::isUserUsingTwoFactorAuthentication($login)) {
        ...
        $view->isAlreadyUsing2fa = \Piwik\Plugins\TwoFactorAuth\TwoFactorAuthentication::isUserUsingTwoFactorAuthentication($login);
    ...
    public function showRecoveryCodes()
    {
        ...
        if (!$regenerateSuccess && !$regenerateError) {
            $email = $container->make(RecoveryCodesShowedEmail::class, array('login' => Piwik::getCurrentUserLogin(), 'emailAddress' => Piwik::getCurrentUserEmail()));
            $email->safeSend();
        }
