...
class Update extends ConsoleCommand
{
    ...
    protected function makeUpdate($doDryRun)
    {
        ...
        if (!DbHelper::isInstalled()) {
            $this->handleCoreError(Piwik::translate('CoreUpdater_EmptyDatabaseError', Config::getInstance()->database['dbname']));
            return;
        }
    ...
    private function handleCoreError($errors, $includeDiyHelp = \false)
    {
        ...
        $output = $this->getOutput();
        $output->writeln(array("", "    [X] " . Piwik::translate('CoreUpdater_CriticalErrorDuringTheUpgradeProcess'), ""));
        foreach ($errors as $errorMessage) {
            $errorMessage = trim($errorMessage);
            $errorMessage = str_replace("\n", "\n    ", $errorMessage);
            $output->writeln("    * {$errorMessage}");
        }
        $output->writeln(array("", "    " . Piwik::translate('CoreUpdater_HelpMessageIntroductionWhenError'), "", "    * " . $this->getUpdateHelpMessage()));
        if ($includeDiyHelp) {
            $output->writeln(array("", "    " . Piwik::translate('CoreUpdater_ErrorDIYHelp'), "", "    * " . Piwik::translate('CoreUpdater_ErrorDIYHelp_1'), "    * " . Piwik::translate('CoreUpdater_ErrorDIYHelp_2'), "    * " . Piwik::translate('CoreUpdater_ErrorDIYHelp_3'), "    * " . Piwik::translate('CoreUpdater_ErrorDIYHelp_4'), "    * " . Piwik::translate('CoreUpdater_ErrorDIYHelp_5')));
        }
    ...
    {
        $output = $this->getOutput();
        $output->writeln(array("", "    [!] " . Piwik::translate('CoreUpdater_WarningMessages'), ""));
    ...
    private function outputUpdaterErrors($errors, $deactivatedPlugins)
    {
        ...
        if (!empty($deactivatedPlugins)) {
            $output->writeln(array("", "    [!] " . Piwik::translate('CoreUpdater_WeAutomaticallyDeactivatedTheFollowingPlugins', implode(', ', $deactivatedPlugins))));
        }
    }
    ...
    private function getPluginsToUpdate($componentsWithUpdateFile)
    {
        $plugins = array();
        foreach ($componentsWithUpdateFile as $componentName => $updates) {
            if ($componentName !== 'core' && 0 !== strpos($componentName, 'log_')) {
    ...
    protected function writeAlertMessageWhenCommandExecutedWithUnexpectedUser()
    {
        ...
        $this->getOutput()->writeln(sprintf("<comment>%s</comment>", Piwik::translate('CoreUpdater_ConsoleUpdateUnexpectedUserWarning', [$processUserAndGroup, $fileOwnerUserAndGroup, Filechecks::getCommandToChangeOwnerOfPiwikFiles()])));
    }
}
