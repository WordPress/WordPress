...
class Goals extends \Piwik\Plugin
{
    ...
    public static function getGoalColumns($idGoal)
    {
        ...
        if ($idGoal === GoalManager::IDGOAL_ORDER) {
            $columns = array_merge($columns, array('revenue_subtotal', 'revenue_tax', 'revenue_shipping', 'revenue_discount'));
        }
    ...
    public function addRecordBuilders(array &$recordBuilders) : void
    {
        ...
        $recordBuilders[] = new ProductRecord(ProductRecord::CATEGORY_FIELD, ProductRecord::ITEMS_CATEGORY_RECORD_NAME, [ProductRecord::CATEGORY2_FIELD, ProductRecord::CATEGORY3_FIELD, ProductRecord::CATEGORY4_FIELD, ProductRecord::CATEGORY5_FIELD]);
    }
    public function addSystemSummaryItems(&$systemSummary)
    {
        $goalModel = new \Piwik\Plugins\Goals\Model();
    ...
    public function addMetrics(MetricsList $metricsList)
    {
        ...
        $goals = Request::processRequest('Goals.getGoals', ['idSite' => $idSite, 'filter_limit' => '-1'], $default = []);
        foreach ($goals as $goal) {
            $custom = new \Piwik\Plugins\Goals\GoalDimension($goal, 'idgoal', 'Conversions goal "' . $goal['name'] . '" (ID ' . $goal['idgoal'] . ' )');
            ...
            $custom = new \Piwik\Plugins\Goals\GoalDimension($goal, 'revenue', 'Revenue goal "' . $goal['name'] . '" (ID ' . $goal['idgoal'] . ' )');
            ...
            $metric->setCategory($custom->getCategoryId());
            $metricsList->addMetric($metric);
            $custom = new \Piwik\Plugins\Goals\GoalDimension($goal, 'visitor_seconds_since_first', 'Days to conversion goal "' . $goal['name'] . '" (ID ' . $goal['idgoal'] . ' )');
            $custom->setType(Dimension::TYPE_NUMBER);
            $metric = new ArchivedMetric($custom, ArchivedMetric::AGGREGATION_SUM);
            $metric->setTranslatedName($custom->getName());
            $metric->setCategory($custom->getCategoryId());
            ...
            $metricsList->addMetric($metric);
            $custom = new \Piwik\Plugins\Goals\GoalDimension($goal, 'visitor_count_visits', 'Visits to conversion goal "' . $goal['name'] . '" (ID ' . $goal['idgoal'] . ' )');
            $custom->setType(Dimension::TYPE_NUMBER);
            $metric = new ArchivedMetric($custom, ArchivedMetric::AGGREGATION_SUM);
            $metric->setTranslatedName($custom->getName());
            $metric->setCategory($custom->getCategoryId());
    ...
    {
        $goalMetricTypes = array('orders' => Dimension::TYPE_NUMBER, 'ecommerce_revenue' => Dimension::TYPE_MONEY, 'quantity' => Dimension::TYPE_NUMBER, 'revenue_subtotal' => Dimension::TYPE_MONEY, 'revenue_tax' => Dimension::TYPE_MONEY, 'revenue_shipping' => Dimension::TYPE_MONEY, 'revenue_discount' => Dimension::TYPE_MONEY, 'avg_order_revenue' => Dimension::TYPE_MONEY, 'items' => Dimension::TYPE_NUMBER);
    ...
    {
        $model = new \Piwik\Plugins\Goals\Model();
    ...
    public function getReportMetadataEnd(&$reports, $info)
    {
        ...
        $goalMetricTypes = ['revenue_per_visit' => Dimension::TYPE_MONEY, 'nb_conversions' => Dimension::TYPE_NUMBER, 'conversion_rate' => Dimension::TYPE_PERCENT, 'revenue' => Dimension::TYPE_MONEY];
        // special goal metrics for Actions page reports
        $pageGoalMetrics = array_merge($goalMetrics, ['nb_conversions_attrib' => Piwik::translate('Goals_ColumnConversions'), 'revenue_attrib' => Piwik::translate('General_ColumnRevenue')]);
        unset($pageGoalMetrics['revenue']);
        $pageGoalProcessedMetrics = array_merge($goalProcessedMetrics, ['nb_conversions_page_rate' => Piwik::translate('Goals_ConversionRatePageViewedBeforeGeneric')]);
        $pageGoalMetricTypes = array_merge($goalMetricTypes, ['nb_conversions_attrib' => Dimension::TYPE_NUMBER, 'revenue_attrib' => Dimension::TYPE_MONEY, 'nb_conversions_page_rate' => Dimension::TYPE_PERCENT]);
        unset($pageGoalMetricTypes['revenue']);
        // special goal metrics for Actions entry page reports
        $entryPageGoalMetrics = array_merge($goalMetrics, ['nb_conversions_entry' => Piwik::translate('Goals_ColumnConversions'), 'revenue_entry' => Piwik::translate('General_ColumnRevenue')]);
        unset($entryPageGoalMetrics['revenue']);
        $entryPageGoalProcessedMetrics = array_merge($goalProcessedMetrics, ['revenue_per_entry' => Piwik::translate('General_ColumnValuePerEntry'), 'nb_conversions_entry_rate' => Piwik::translate('General_ColumnConversionRate')]);
        $entryPageGoalMetricTypes = array_merge($goalMetricTypes, ['nb_conversions_entry' => Dimension::TYPE_NUMBER, 'revenue_entry' => Dimension::TYPE_MONEY, 'revenue_per_entry' => Dimension::TYPE_NUMBER, 'nb_conversions_entry_rate' => Dimension::TYPE_PERCENT]);
        ...
        if ($idGoal === Piwik::LABEL_ID_GOAL_IS_ECOMMERCE_ORDER || $idGoal === Piwik::LABEL_ID_GOAL_IS_ECOMMERCE_CART) {
            $extraEcommerceProcessedMetrics = ['avg_order_revenue' => Piwik::translate('General_AverageOrderValue'), 'items' => Piwik::translate('General_PurchasedProducts')];
            $extraEcommerceMetricTypes = ['avg_order_revenue' => Dimension::TYPE_MONEY, 'itmes' => Dimension::TYPE_NUMBER];
    ...
    public function fetchGoalsFromDb(&$array, $idSite)
    {
        ...
        $array['goals'] = \Piwik\Plugins\Goals\API::getInstance()->getGoals($idSite);
    }
