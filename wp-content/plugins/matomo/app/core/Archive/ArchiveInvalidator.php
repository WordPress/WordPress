...
class ArchiveInvalidator
{
     ...
     * @throws \Exception
     */
    public function markArchivesAsInvalidated(array $idSites, array $dates, $period, ?Segment $segment = null, bool $cascadeDown = \false, bool $forceInvalidateNonexistentRanges = \false, ?string $name = null, bool $ignorePurgeLogDataDate = \false, bool $doNotCreateInvalidations = \false)
    {
        $plugin = null;
        ...
        if ($isInvalidatingDays && $isNotInvalidatingSegment) {
            $hasDeletedAny = \false;
    ...
    {
        return $periodObj->getDateStart()->toString('Y') == $periodObj->getDateEnd()->toString('Y') && $periodObj->getDateStart()->toString('m') == $periodObj->getDateEnd()->toString('m');
    }
    ...
    {
        if ($period->getLabel() == 'year' || $period->getLabel() == 'range' || !Period\Factory::isPeriodEnabledForAPI($period->getParentPeriodLabel())) {
            return;
        }
    ...
    public function scheduleReArchiving($idSites, ?string $pluginName = null, $report = null, ?Date $startDate = null, ?Segment $segment = null)
    {
        ...
        try {
            ...
            $reArchiveList->add(json_encode(['idSites' => $idSites, 'pluginName' => $pluginName, 'report' => $report, 'startDate' => $startDate ? $startDate->getTimestamp() : null, 'segment' => $segment ? $segment->getOriginalString() : null]));
        } catch (\Throwable $ex) {
    ...
    public function applyScheduledReArchiving()
    {
        ...
        foreach ($items as $item) {
            try {
                $entry = @json_decode($item, \true);
                if (empty($entry)) {
                    continue;
                }
                $idSites = Site::getIdSitesFromIdSitesString($entry['idSites']);
                $this->reArchiveReport($idSites, $entry['pluginName'], $entry['report'], !empty($entry['startDate']) ? Date::factory((int) $entry['startDate']) : null, !empty($entry['segment']) ? new Segment($entry['segment'], $idSites) : null);
            } catch (\Throwable $ex) {
                $this->logger->info("Failed to create invalidations for report re-archiving (idSites = {idSites}, pluginName = {pluginName}, report = {report}, startDate = {startDateTs}): {ex}", ['idSites' => json_encode($entry['idSites']), 'pluginName' => $entry['pluginName'], 'report' => $entry['report'], 'startDateTs' => $entry['startDate'], 'ex' => $ex]);
            } finally {
    ...
    public function removeInvalidationsFromDistributedList($idSites, $pluginName = null, $report = null)
    {
        ...
        $idSites = is_array($idSites) ? $idSites : ($idSites !== \true ? [$idSites] : []);
        foreach ($entries as $index => $entry) {
            $entry = @json_decode($entry, \true);
            if (empty($entry)) {
                unset($entries[$index]);
                continue;
            }
            $entryPluginName = $entry['pluginName'];
            if (!empty($pluginName) && $pluginName != $entryPluginName) {
                continue;
            }
            $entryReport = $entry['report'];
            if (!empty($pluginName) && !empty($report) && $report != $entryReport) {
                continue;
            }
    ...
    private function removeDatesThatHaveBeenPurged($dates, $period, InvalidationResult $invalidationInfo, $ignorePurgeLogDataDate)
    {
        ...
        foreach ($dates as $date) {
            ...
            if ($invalidationInfo->minimumDateWithLogs && !$ignorePurgeLogDataDate && ($periodObj->getDateEnd()->isEarlier($invalidationInfo->minimumDateWithLogs) || $periodObj->getDateStart()->isEarlier($invalidationInfo->minimumDateWithLogs))) {
    ...
    {
        if ($period === 'range' && strpos($date, ',') === \false) {
            $date = $date . ',' . $date;
