...
class PluginsArchiver
{
    ...
    public function callAggregateAllPlugins($visits, $visitsConverted, $forceArchivingWithoutVisits = \false)
    {
        ...
        foreach ($archivers as $pluginName => $archiverClass) {
            ...
            if (!empty($archiveOnlyReports) && $archiveOnlyPlugin != $pluginName) {
                continue;
            }
            ...
            if ($this->shouldProcessReportsForPlugin($pluginName)) {
                ...
                try {
                    ...
                    Log::debug("PluginsArchiver::%s: %s while archiving %s reports for plugin '%s' %s.", __FUNCTION__, $timer->getMemoryLeak(), $this->params->getPeriod()->getLabel(), $pluginName, $this->params->getSegment() ? sprintf("(for segment = '%s')", $this->params->getSegment()->getString()) : '');
                } catch (Exception $e) {
                    throw new \Piwik\ArchiveProcessor\PluginsArchiverException($e->getMessage() . " - in plugin {$pluginName}.", $e->getCode(), $e);
                } finally {
    ...
    private static function getPluginArchiverClass(string $pluginName) : ?string
    {
        ...
        if (class_exists($klassName) && is_subclass_of($klassName, 'Piwik\\Plugin\\Archiver')) {
            return $klassName;
        }
