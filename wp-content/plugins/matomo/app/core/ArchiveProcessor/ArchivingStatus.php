...
class ArchivingStatus
{
    ...
    public const DEFAULT_ARCHIVING_TTL = 7200;
    // 2 hours
    /**
     * @var LockBackend
     */
    ...
    public function archiveStarted(\Piwik\ArchiveProcessor\Parameters $params)
    {
     ...
     * @return Lock
     */
    private function makeArchivingLock(\Piwik\ArchiveProcessor\Parameters $params)
    {
        $doneFlag = \Piwik\ArchiveProcessor\Rules::getDoneStringFlagFor([$params->getSite()->getId()], $params->getSegment(), $params->getPeriod()->getLabel(), $params->getRequestedPlugin());
        return $this->makeArchivingLockFromDoneFlag($params->getSite()->getId(), $params->getSite()->getId(), $params->getPeriod()->getDateStart()->toString(), $params->getPeriod()->getDateEnd()->toString(), $doneFlag);
    }
