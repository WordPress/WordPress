...
class LogAggregator
{
    ...
    public function cleanup()
    {
        if (!$this->segment->isEmpty() && $this->isSegmentCacheEnabled()) {
            ...
            if ($logTablesProvider->getLogTable($segmentTable)) {
                $logTablesProvider->setTempTable(null);
                // no longer available
            }
        }
    }
    ...
    private function createTemporaryTable($unprefixedSegmentTableName, $segmentSelectSql, $segmentSelectBind)
    {
        ...
        if ($this->doesSegmentTableExist($table)) {
            return;
            // no need to create the table, it was already created... better to have a select vs unneeded create table
        }
        $engine = '';
    ...
    public function generateQuery($select, $from, $where, $groupBy, $orderBy, $limit = 0, $offset = 0)
    {
        ...
        if (!$this->segment->isEmpty() && $this->isSegmentCacheEnabled()) {
            ...
            $logTablesProvider->setTempTable(new \Piwik\DataAccess\LogTableTemporary($segmentTable));
            ...
            foreach ($logTablesProvider->getAllLogTables() as $logTable) {
                ...
                foreach ($from as $fromJoin) {
                    if (!empty($fromJoin['table']) && $fromJoin['table'] === $logTable->getName() && !empty($fromJoin['join']) && strtoupper($fromJoin['join']) === 'RIGHT JOIN') {
                        continue 2;
                    }
                }
    ...
    {
        $bind = [$this->dateStart->toString(Date::DATE_TIME_FORMAT), $this->dateEnd->toString(Date::DATE_TIME_FORMAT)];
    ...
    public function queryEcommerceItems($dimension)
    {
        $query = $this->generateQuery(
            // SELECT ...
            implode(', ', array("log_action.name AS label", sprintf("log_conversion_item.%s AS labelIdAction", $dimension), sprintf('%s AS `%d`', self::getSqlRevenue('SUM(log_conversion_item.quantity * log_conversion_item.price)'), Metrics::INDEX_ECOMMERCE_ITEM_REVENUE), sprintf('%s AS `%d`', self::getSqlRevenue('SUM(log_conversion_item.quantity)'), Metrics::INDEX_ECOMMERCE_ITEM_QUANTITY), sprintf('%s AS `%d`', self::getSqlRevenue('SUM(log_conversion_item.price)'), Metrics::INDEX_ECOMMERCE_ITEM_PRICE), sprintf('COUNT(distinct log_conversion_item.idorder) AS `%d`', Metrics::INDEX_ECOMMERCE_ORDERS), sprintf('COUNT(distinct log_conversion_item.idvisit) AS `%d`', Metrics::INDEX_NB_VISITS), sprintf('CASE log_conversion_item.idorder WHEN \'0\' THEN %d ELSE %d END AS ecommerceType', GoalManager::IDGOAL_CART, GoalManager::IDGOAL_ORDER))),
            // FROM ...
            array("log_conversion_item", array("table" => "log_action", "joinOn" => sprintf("log_conversion_item.%s = log_action.idaction", $dimension))),
            // WHERE ... AND ...
            implode(' AND ', array('log_conversion_item.server_time >= ?', 'log_conversion_item.server_time <= ?', 'log_conversion_item.idsite IN (' . Common::getSqlStringFieldsArray($this->sites) . ')', 'log_conversion_item.deleted = 0')),
            // GROUP BY ...
     ...
     */
    public function queryConversionsByDimension($dimensions = array(), $where = \false, $additionalSelects = array(), $extraFrom = [], $rankingQuery = \false, $rankingQueryGenerate = \false, $forceSiteDateIndex = \false)
    {
    ...
    public function queryConversionsByEntryPageView(string $linkField, int $rankingQueryLimit = 0)
    {
        ...
        $from = [$tableName, ["table" => "log_visit", "joinOn" => "log_visit.idvisit = log_conversion.idvisit"], ["table" => "log_action", "joinOn" => "log_action.idaction = log_visit." . $linkField]];
    ...
    public static function getSelectsFromRangedColumn($column, $ranges, $table, $selectColumnPrefix, $restrictToReturningVisitors = \false)
    {
        ...
        foreach ($ranges as $gap) {
            if (count($gap) == 2) {
                ...
                $selects[] = "sum(case when {$tableColumn} between {$lowerBound} and {$upperBound} {$extraCondition}" . " then 1 else 0 end) as `{$selectAs}`";
            } else {
                $lowerBound = $gap[0];
