...
class Report
{
    ...
    public function getMetricSemanticTypes() : array
    {
        ...
        foreach ($allMetrics as $metric) {
            $metricName = $metric instanceof \Piwik\Plugin\Metric ? $metric->getName() : $metric;
            if ($metricName == 'label' || !empty($metricTypes[$metricName])) {
                continue;
            }
    ...
    protected function getMetricsDocumentation()
    {
        ...
        foreach ($this->metrics as $metric) {
            ...
            } elseif ($metric instanceof \Piwik\Plugin\Metric) {
                $name = $metric->getName();
        ...
        foreach ($processedMetrics as $processedMetric) {
            ...
            } elseif ($processedMetric instanceof \Piwik\Plugin\Metric) {
    ...
    {
        $provider = new \Piwik\Plugin\ReportsProvider();
        $reports = $provider->getAllReports();
        foreach ($reports as $report) {
            if (!$report->isSubtableReport() && $report->getDimension() && $report->getDimension()->getId() == $dimension->getId()) {
                return $report;
            }
        }
        return null;
    ...
    public function getProcessedMetricsById()
    {
        ...
        $result = array();
        foreach ($processedMetrics as $processedMetric) {
            if ($processedMetric instanceof \Piwik\Plugin\ProcessedMetric) {
                // instanceof check for backwards compatibility
                $result[$processedMetric->getName()] = $processedMetric;
            } elseif ($processedMetric instanceof \Piwik\Plugin\ArchivedMetric && $processedMetric->getType() !== Dimension::TYPE_NUMBER && $processedMetric->getType() !== Dimension::TYPE_FLOAT && $processedMetric->getType() !== Dimension::TYPE_BOOL && $processedMetric->getType() !== Dimension::TYPE_ENUM) {
