...
class ComputedMetric extends \Piwik\Plugin\ProcessedMetric
{
    ...
    public function __construct($metric1, $metric2, $aggregation = 'avg')
    {
        ...
        if ($aggregation === \Piwik\Plugin\ComputedMetric::AGGREGATION_AVG) {
            $this->name = 'avg_' . $nameShort1 . '_per_' . $nameShort2;
        } elseif ($aggregation === \Piwik\Plugin\ComputedMetric::AGGREGATION_RATE) {
    ...
    private function getDetectedType()
    {
        if (!$this->type) {
            if ($this->aggregation === self::AGGREGATION_RATE) {
                ...
                $this->type = Dimension::TYPE_NUMBER;
                // default to number
    ...
    public function getTranslatedName()
    {
        if (!$this->translatedName) {
            ...
            if ($this->aggregation === self::AGGREGATION_AVG) {
                if ($metric1 && $metric1 instanceof \Piwik\Plugin\ArchivedMetric && $metric2 && $metric2 instanceof \Piwik\Plugin\ArchivedMetric) {
