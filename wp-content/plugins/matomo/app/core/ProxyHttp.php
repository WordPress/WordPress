...
class ProxyHttp
{
    ...
    {
        return \Piwik\Url::getCurrentScheme() === 'https';
    }
    /**
    ...
    public static function serverStaticFile($file, $contentType, $expireFarFutureDays = 100, $byteStart = \false, $byteEnd = \false, $filename = \false)
    {
        ...
        if (empty($file) || !file_exists($file)) {
            \Piwik\Common::sendResponseCode(404);
            return;
        }
        if (!is_readable($file)) {
        ...
        $modifiedSince = \Piwik\Http::getModifiedSinceHeader();
        ...
        if ($expireFarFutureDays) {
            ...
            \Piwik\Common::sendHeader(self::getExpiresHeaderForFutureDay($expireFarFutureDays));
        }
        // Return 304 if the file has not modified since
        if ($modifiedSince === $lastModified) {
            \Piwik\Common::sendResponseCode(304);
            return;
        }
        ...
        \Piwik\Common::sendHeader('Last-Modified: ' . $lastModified);
        if (!$phpOutputCompressionEnabled) {
            \Piwik\Common::sendHeader('Content-Length: ' . ($byteEnd - $byteStart));
        }
        if (!empty($contentType)) {
            \Piwik\Common::sendHeader('Content-Type: ' . $contentType);
        }
        if ($compressed) {
            \Piwik\Common::sendHeader('Content-Encoding: ' . $encoding);
        }
