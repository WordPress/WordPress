...
class GoalManager
{
    ...
    {
        $websiteAttributes = \Piwik\Tracker\Cache::getCacheWebsiteAttributes($idSite);
    ...
    private function detectNumericGoalMatch($goal, \Piwik\Tracker\Action $action, VisitProperties $visitProperties, \Piwik\Tracker\Request $request)
    {
    ...
    public function detectGoalId($idSite, \Piwik\Tracker\Request $request)
    {
        if (!Common::isGoalPluginEnabled()) {
            return null;
        }
        ...
        $goal['url'] = \Piwik\Tracker\PageUrl::excludeQueryParametersFromUrl($url, $idSite);
        return $goal;
    }
    /**
    ...
    protected function recordEcommerceGoal(VisitProperties $visitProperties, \Piwik\Tracker\Request $request, $conversion, $action)
    {
        ...
        $visitor = \Piwik\Tracker\Visitor::makeFromVisitProperties($visitProperties, $request);
        ...
        foreach ($items as $item) {
            $itemsCount += $item[\Piwik\Tracker\GoalManager::INTERNAL_ITEM_QUANTITY];
        }
     ...
     * @return array|bool
     */
    private function getEcommerceItemsFromRequest(\Piwik\Tracker\Request $request)
    {
    ...
    private function getCleanedEcommerceItems($items)
    {
        ...
        foreach ($cleanedItems as $item) {
            ...
            $actionsToLookup[] = array(trim($sku), \Piwik\Tracker\Action::TYPE_ECOMMERCE_ITEM_SKU);
            $name = is_array($name_check) ? join(',', $name_check) : $name_check;
            $actionsToLookup[] = array(trim($name), \Piwik\Tracker\Action::TYPE_ECOMMERCE_ITEM_NAME);
            // Only one category
            if (!is_array($category)) {
                ...
                foreach ($category as $productCategory) {
                    ...
                    $actionsToLookup[] = array($productCategory, \Piwik\Tracker\Action::TYPE_ECOMMERCE_ITEM_CATEGORY);
                }
            }
        ...
        $actionsLookedUp = \Piwik\Tracker\TableLogAction::loadIdsAction($actionsToLookupAllItems);
     ...
     */
    private function triggerHookOnDimensions(\Piwik\Tracker\Request $request, $dimensions, $hook, $visitor, $action, $valuesToUpdate)
    {
    ...
    private function getGoalFromVisitor(VisitProperties $visitProperties, \Piwik\Tracker\Request $request, $action)
    {
        $lastVisitTime = $visitProperties->getProperty('visit_last_action_time');
        if (!$lastVisitTime) {
            $lastVisitTime = $request->getCurrentTimestamp();
            // fallback in case visit_last_action_time is not set
        }
        ...
        $goal = array('idvisit' => $visitProperties->getProperty('idvisit'), 'idvisitor' => $visitProperties->getProperty('idvisitor'), 'server_time' => $lastVisitTime);
