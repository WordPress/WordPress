...
class Request
{
    ...
    protected function replaceUnsupportedUtf8Chars($value, $key = \false)
    {
        ...
        if ('utf8mb4' === $charset) {
            return $value;
            // no need to replace anything if utf8mb4 is supported
        }
        if (is_string($value) && preg_match('/[\\x{10000}-\\x{10FFFF}]/u', $value)) {
            Common::printDebug("Unsupported character detected in {$key}. Replacing with �");
    ...
    public static function authenticateSuperUserOrAdminOrWrite($tokenAuth, $idSite)
    {
        ...
        if (!empty($idSite) && $idSite > 0) {
            $website = \Piwik\Tracker\Cache::getCacheWebsiteAttributes($idSite);
    ...
    {
        $excludedRequests = \Piwik\Tracker\TrackerConfig::getConfigValue('exclude_requests', $this->getIdSiteIfExists());
        if (!empty($excludedRequests)) {
            $excludedRequests = explode(',', $excludedRequests);
            $pattern = '/^(.+?)(' . SegmentExpression::MATCH_EQUAL . '|' . SegmentExpression::MATCH_NOT_EQUAL . '|' . SegmentExpression::MATCH_CONTAINS . '|' . SegmentExpression::MATCH_DOES_NOT_CONTAIN . '|' . preg_quote(SegmentExpression::MATCH_STARTS_WITH) . '|' . preg_quote(SegmentExpression::MATCH_ENDS_WITH) . '){1}(.*)/';
    ...
    {
        $localTimes = array('h' => (string) Common::getRequestVar('h', $this->getCurrentDate("H"), 'int', $this->params), 'i' => (string) Common::getRequestVar('m', $this->getCurrentDate("i"), 'int', $this->params), 's' => (string) Common::getRequestVar('s', $this->getCurrentDate("s"), 'int', $this->params));
    ...
    protected function isTimestampValid($time, $now = null)
    {
        ...
        return $time <= $now && $time > $now - 20 * 365 * 86400;
    }
    /**
     * @internal
     * @ignore
    ...
    {
        return \Piwik\Tracker\TrackerConfig::getConfigValue('use_third_party_id_cookie', $this->getIdSiteIfExists());
    }
    ...
    {
        $cookie = new Cookie($this->getCookieName(), $this->getCookieExpire(), $this->getCookiePath());
    ...
    {
        return \Piwik\Tracker\TrackerConfig::getConfigValue('cookie_name', $this->getIdSiteIfExists());
    }
    protected function getCookieExpire()
    {
        return $this->getCurrentTimestamp() + \Piwik\Tracker\TrackerConfig::getConfigValue('cookie_expire', $this->getIdSiteIfExists());
    }
    protected function getCookiePath()
    {
        return \Piwik\Tracker\TrackerConfig::getConfigValue('cookie_path', $this->getIdSiteIfExists());
    }
    protected function getCookieDomain()
    {
        return \Piwik\Tracker\TrackerConfig::getConfigValue('cookie_domain', $this->getIdSiteIfExists());
    }
    /**
