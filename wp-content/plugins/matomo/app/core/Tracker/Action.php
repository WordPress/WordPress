...
abstract class Action
{
    ...
    public const TYPE_EVENT = 10;
    // Alias TYPE_EVENT_CATEGORY
    public const TYPE_EVENT_CATEGORY = 10;
    public const TYPE_EVENT_ACTION = 11;
    public const TYPE_EVENT_NAME = 12;
    public const TYPE_CONTENT = 13;
    // Alias TYPE_CONTENT_NAME
    ...
    private static $factoryPriority = array(self::TYPE_PAGE_URL, self::TYPE_SITE_SEARCH, self::TYPE_CONTENT, self::TYPE_EVENT, self::TYPE_OUTLINK, self::TYPE_DOWNLOAD);
    /**
    ...
    public static function factory(\Piwik\Tracker\Request $request)
    {
        ...
        return new \Piwik\Tracker\ActionPageview($request);
    }
    /**
    ...
    {
        $this->actionName = \Piwik\Tracker\PageUrl::cleanupString((string) $name);
    }
    protected function setActionUrl($url)
    {
        $this->rawActionUrl = \Piwik\Tracker\PageUrl::getUrlIfLookValid($url);
        $url2 = \Piwik\Tracker\PageUrl::excludeQueryParametersFromUrl($url, $this->request->getIdSite());
        $this->actionUrl = \Piwik\Tracker\PageUrl::getUrlIfLookValid($url2);
        if ($url != $this->rawActionUrl) {
            $this->logger->debug(' Before was "{rawActionUrl}"', ['rawActionUrl' => $this->rawActionUrl]);
            $this->logger->debug(' After is "{url2}"', ['url2' => $url2]);
        }
    }
    protected function setActionUrlWithoutExcludingParameters($url)
    {
        $url = \Piwik\Tracker\PageUrl::getUrlIfLookValid($url);
        $this->rawActionUrl = $url;
        $this->actionUrl = $url;
    }
    protected abstract function getActionsToLookup();
    protected function getUrlAndType()
    {
        $url = $this->getActionUrl();
        if (!empty($url)) {
            // normalize urls by stripping protocol and www
            $url = \Piwik\Tracker\PageUrl::normalizeUrl($url);
    ...
    public function record(\Piwik\Tracker\Visitor $visitor, $idReferrerActionUrl, $idReferrerActionName)
    {
        ...
        $this->logger->debug("Inserted new action: {action}", ['action' => var_export($visitActionDebug, \true)]);
    }
    public function writeDebugInfo()
    {
        ...
        $url = $this->getActionUrl();
        $this->logger->debug('Action is a {type}, Action name = {name}, Action URL = {url}', ['type' => $type, 'name' => $name, 'url' => $url]);
        return \true;
    }
