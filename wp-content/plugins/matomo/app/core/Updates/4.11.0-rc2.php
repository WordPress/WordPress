...
class Updates_4_11_0_rc2 extends PiwikUpdates
{
    ...
    public function getMigrations(Updater $updater)
    {
        ...
        return [$this->migration->db->dropColumn('user', 'invite_status'), $this->migration->db->addColumns('user', ['invite_token' => 'VARCHAR(191) DEFAULT null']), $this->migration->db->addColumns('user', ['invited_by' => 'VARCHAR(100) DEFAULT null']), $this->migration->db->addColumns('user', ['invite_expired_at' => 'TIMESTAMP null DEFAULT null']), $this->migration->db->addColumns('user', ['invite_accept_at' => 'TIMESTAMP null DEFAULT null'])];
    }
    public function doUpdate(Updater $updater)
    {
        ...
        if (!empty($this->pendingUsers)) {
            foreach ($this->pendingUsers as $user) {
                ...
                // send email
                $email = StaticContainer::getContainer()->make(UserInviteEmail::class, ['currentUser' => Piwik::getCurrentUserLogin(), 'invitedUser' => $user, 'siteName' => $siteName, 'token' => $generatedToken, 'expiryInDays' => 7]);
                $email->safeSend();
            }
        }
    }
