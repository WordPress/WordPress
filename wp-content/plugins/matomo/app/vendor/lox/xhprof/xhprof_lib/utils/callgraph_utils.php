...
namespace {
    ...
    $xhprof_legal_image_types = array(
        ...
        "png" => 1,
        "svg" => 1,
        // support scalable vector graphic
        "ps" => 1,
    );
    /**
        ...
        if ($critical_path) {
            ...
            while ($node) {
                ...
                if (isset($children_table[$node])) {
                    ...
                    if ($max_child !== null) {
                        $path[$max_child] = \true;
                        $path_edges[\xhprof_build_parent_child_key($node, $max_child)] = \true;
                    }
                    $node = $max_child;
                } else {
                    $node = null;
    ...
    function xhprof_render_diff_image($xhprof_runs_impl, $run1, $run2, $type, $threshold, $source)
    {
        ...
        $script = \xhprof_generate_dot_script($run_delta, $threshold, $source, null, null, \true, $symbol_tab1, $symbol_tab2);
