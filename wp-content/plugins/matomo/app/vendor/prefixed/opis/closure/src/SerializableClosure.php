 ...
 * =========================================================================== */
namespace Matomo\Dependencies\Opis\Closure;
...
class SerializableClosure implements Serializable
{
    ...
    public function unserialize($data)
    {
        ...
        if (static::$securityProvider !== null) {
            if ($data[0] !== '@') {
                throw new SecurityException("The serialized closure is not signed. " . "Make sure you use a security provider for both serialization and unserialization.");
            }
            if ($data[1] !== '{') {
            ...
            do {
                ...
                foreach ($reflection->getProperties() as $property) {
                    ...
                    if ($item instanceof SerializableClosure || $item instanceof SelfReference && $item->hash === $this->code['self']) {
                        $this->code['objects'][] = array('instance' => $data, 'property' => $property, 'object' => $item instanceof SelfReference ? $this : $item);
