...
?>
<div data-css-class="woof_acf_select_container" class="woof_acf_select_container woof_container woof_container_<?php echo esc_attr($meta_key) ?>  woof_container_<?php echo esc_attr( $meta_key) ?>">
    <div class="woof_container_inner">
        <div class="woof_container_inner woof_container_inner_meta_select">
            ...
            <div class="<?php echo esc_attr($css_classes) ?>">
                <?php $meta_id = 'woof_acf_select_' . $meta_key ?>
                <label class="woof_wcga_label_hide"  for="<?php echo esc_attr($meta_id) ?>"><?php echo esc_html(WOOF_HELPER::wpml_translate(null, $meta_title)) ?></label>				
                <select id="<?php echo esc_attr($meta_id) ?>" class="woof_acf_select woof_acf_select_<?php echo esc_attr($meta_key) ?>" name="<?php echo esc_attr($meta_key) ?>">
                    <option value="0"><?php echo esc_html(WOOF_HELPER::wpml_translate(null, $meta_title)) ?></option>

                    <?php if (!empty($all_options)): ?>
                        <?php foreach ($all_options as $key => $option) : ?>

                            <option <?php if ($show_count AND $option['count'] == 0 AND $key != $woof_value): ?>disabled=""<?php endif; ?> value="<?php echo esc_attr($key) ?>" <?php selected($key == $woof_value) ?>>
                                <?php
