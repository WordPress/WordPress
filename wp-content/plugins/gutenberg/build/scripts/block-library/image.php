...
function gutenberg_render_block_core_image( $attributes, $content, $block ) {
	...
	$lightbox_settings = gutenberg_block_core_image_get_lightbox_settings( $block->parsed_block );
	...
	if (
		 ...
		 * introduced.
		 */
		add_filter( 'render_block_core/image', 'gutenberg_block_core_image_render_lightbox', 15, 2 );
	} else {
		/*
		 * Remove the filter if previously added by other Image blocks.
		 */
		remove_filter( 'render_block_core/image', 'gutenberg_block_core_image_render_lightbox', 15 );
	}
 ...
 */
function gutenberg_block_core_image_get_lightbox_settings( $block ) {
	...
	if ( ! isset( $lightbox_settings ) ) {
		...
		// `gutenberg_get_global_settings` will return the whole `theme.json` structure in
 ...
 */
function gutenberg_block_core_image_render_lightbox( $block_content, $block ) {
	/*
 ...
 * @since 6.5.0
 */
function gutenberg_block_core_image_print_lightbox_overlay() {
